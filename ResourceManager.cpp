...
static void ResourceManagerDetectionCallback(void * this_ptr, unsigned int update_reason)
{
    ...
    this_obj->SignalDetectionUpdate(update_reason);
...
\*---------------------------------------------------------*/
void ResourceManager::RegisterDetectionCallback(DetectionCallback new_callback, void * new_callback_arg)
{
    ...
    for(size_t idx = 0; idx < DetectionCallbacks.size(); idx++)
    {
        if(DetectionCallbacks[idx] == new_callback && DetectionCallbackArgs[idx] == new_callback_arg)
        {
            ...
            LOG_TRACE("[%s] Tried to register an already registered detection callback, skipping.  Total callbacks registered: %d", RESOURCEMANAGER, DetectionCallbacks.size());
    ...
    LOG_TRACE("[%s] Registered detection callback.  Total callbacks registered: %d", RESOURCEMANAGER, DetectionCallbacks.size());
}
...
void ResourceManager::UnregisterDetectionCallback(DetectionCallback callback, void *callback_arg)
{
    DetectionCallbackMutex.lock();
    ...
    LOG_TRACE("[%s] Unregistered detection callback.  Total callbacks registered: %d", RESOURCEMANAGER, DetectionCallbacks.size());
}
...
\*---------------------------------------------------------*/
void ResourceManager::SignalDetectionUpdate(unsigned int update_reason)
{
    DetectionCallbackMutex.lock();
    ...
    LOG_TRACE("[%s] Detection update signalled: %d.", RESOURCEMANAGER, update_reason);
}
