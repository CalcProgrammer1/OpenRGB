...
| Translation Strings                                                       |
\*-------------------------------------------------------------------------*/
const char *I2C_ERR_WIN = QT_TRANSLATE_NOOP(
    "ResourceManager",
    "<h2>Some internal devices may not be detected:</h2>"
    "<p>One or more I2C or SMBus interfaces failed to initialize.</p>"
    "<p><b>RGB DRAM modules, some motherboards' onboard RGB lighting, and RGB Graphics Cards, will "
    "not be available in OpenRGB</b> without I2C or SMBus.</p>"
    ...
    "you.</h3>");
const char *I2C_ERR_LINUX = QT_TRANSLATE_NOOP(
    "ResourceManager",
    "<h2>Some internal devices may not be detected:</h2>"
    "<p>One or more I2C or SMBus interfaces failed to initialize.</p>"
    "<p><b>RGB DRAM modules, some motherboards' onboard RGB lighting, and RGB Graphics Cards, will "
...
const char *UDEV_MISSING
    = QT_TRANSLATE_NOOP("ResourceManager",
                        "<h2>WARNING:</h2>"
                        "<p>The OpenRGB udev rules are not installed.</p>"
                        "<p>Most devices will not be available unless running OpenRGB as root.</p>"
                        "<p>If using AppImage, Flatpak, or self-compiled versions of OpenRGB you "
                        "must install the udev rules manually</p>"
                        "<p>See <a href='https://openrgb.org/udev'>https://openrgb.org/udev</a> to "
                        "install the udev rules manually</p>");
const char *UDEV_MUTLI = QT_TRANSLATE_NOOP(
    "ResourceManager",
    "<h2>WARNING:</h2>"
    "<p>Multiple OpenRGB udev rules are installed.</p>"
    "<p>The udev rules file 60-openrgb.rules is installed in both /etc/udev/rules.d and "
    "/usr/lib/udev/rules.d.</p>"
...
{
    LOG_INFO(
        "[ResourceManager] Registering I2C interface: %s Device %04X:%04X Subsystem: %04X:%04X",
        bus->device_name,
        bus->pci_vendor,
        bus->pci_device,
...
void ResourceManager::RegisterI2CDIMMDeviceDetector(std::string name,
...
void ResourceManager::DeviceListChanged()
{
    ...
    for (std::size_t callback_idx = 0;
         callback_idx < (unsigned int) DeviceListChangeCallbacks.size();
         callback_idx++) {
...
void ResourceManager::DetectionProgressChanged()
{
    ...
    for (std::size_t callback_idx = 0;
         callback_idx < (unsigned int) DetectionProgressCallbacks.size();
         callback_idx++) {
...
void ResourceManager::I2CBusListChanged()
{
    ...
    \*-------------------------------------------------*/
    for (std::size_t callback_idx = 0;
         callback_idx < (unsigned int) I2CBusListChangeCallbacks.size();
         callback_idx++) {
...
void ResourceManager::Cleanup()
{
    ...
    for (std::size_t hw_controller_idx = 0; hw_controller_idx < rgb_controllers_hw.size();
         hw_controller_idx++) {
...
bool ResourceManager::ProcessPreDetection()
{
    ...
    for (std::size_t callback_idx = 0; callback_idx < DetectionStartCallbacks.size();
         callback_idx++) {
    ...
    if (detection_enabled) {
        ...
        LOG_INFO("[ResourceManager] Initializing HID interfaces: %s",
                 ((hid_status == 0) ? "Success" : "Failed"));
...
void ResourceManager::DetectDevicesCoroutine()
{
    ...
    for (std::size_t size_idx = 0; size_idx < (unsigned int) detection_size_entry_used.size();
         size_idx++) {
    ...
    percent_denominator = (float) (i2c_device_detectors.size() + i2c_dimm_device_detectors.size()
                                   + i2c_pci_device_detectors.size() + device_detectors.size())
    ...
    for (unsigned int i2c_bus_detector_idx = 0;
         i2c_bus_detector_idx < (unsigned int) i2c_bus_detectors.size()
    ...
    for (unsigned int i2c_detector_idx = 0;
         i2c_detector_idx < (unsigned int) i2c_device_detectors.size()
         && detection_is_required.load();
         i2c_detector_idx++) {
    ...
    LOG_INFO("------------------------------------------------------");
    for (unsigned int bus = 0; bus < busses.size() && IsAnyDimmDetectorEnabled(detector_settings);
         bus++) {
        IF_DRAM_SMBUS(busses[bus]->pci_vendor, busses[bus]->pci_device)
        {
            ...
            for (uint8_t spd_addr = 0x50; spd_addr < 0x58; spd_addr++) {
                ...
                if (spd.is_valid()) {
                    ...
                    LOG_INFO("[ResourceManager] Detected occupied slot %d, bus %d, type %s",
                             spd_addr - 0x50 + 1,
                             bus,
                             spd_memory_type_name[dimm_type]);
            ...
            for (unsigned int i2c_detector_idx = 0;
                 i2c_detector_idx < i2c_dimm_device_detectors.size() && detection_is_required.load();
                 i2c_detector_idx++) {
                if (i2c_dimm_device_detectors[i2c_detector_idx].dimm_type == dimm_type
                    && is_jedec_in_slots(slots,
                                         i2c_dimm_device_detectors[i2c_detector_idx].jedec_id)) {
                    ...
                    if (this_device_enabled) {
                        ...
                        std::vector<SPDWrapper *> matching_slots
                            = slots_with_jedec(slots,
                                               i2c_dimm_device_detectors[i2c_detector_idx].jedec_id);
                        i2c_dimm_device_detectors[i2c_detector_idx].function(busses[bus],
                                                                             matching_slots);
                    }
                ...
                | Update detection percent                          |
                \*-------------------------------------------------*/
                percent = (i2c_device_detectors.size() + i2c_detector_idx + 1.0f)
                          / percent_denominator;
    ...
    LOG_INFO("------------------------------------------------------");
    for (unsigned int i2c_detector_idx = 0;
         i2c_detector_idx < (unsigned int) i2c_pci_device_detectors.size()
         && detection_is_required.load();
         i2c_detector_idx++) {
        ...
        | Update detection percent                          |
        \*-------------------------------------------------*/
        percent = (i2c_device_detectors.size() + i2c_dimm_device_detectors.size() + i2c_detector_idx
                   + 1.0f)
                  / percent_denominator;
    ...
    if (hid_safe_mode) {
        ...
        for (unsigned int hid_detector_idx = 0;
             hid_detector_idx < (unsigned int) hid_device_detectors.size()
             && detection_is_required.load();
             hid_detector_idx++) {
            HIDDeviceDetectorBlock &detector = hid_device_detectors[hid_detector_idx];
            hid_devices = hid_enumerate(detector.vid, detector.pid);

            LOG_VERBOSE("[ResourceManager] Trying to run detector for [%s] (for %04x:%04x)",
                        detector.name.c_str(),
                        detector.vid,
                        detector.pid);
        ...
        while (current_hid_device) {
            ...
            \*-----------------------------------------------------------------------------*/
            for (unsigned int hid_detector_idx = 0;
                 hid_detector_idx < (unsigned int) hid_wrapped_device_detectors.size()
                 && detection_is_required.load();
                 hid_detector_idx++) {
            ...
            percent = (i2c_device_detectors.size() + i2c_dimm_device_detectors.size()
                       + i2c_pci_device_detectors.size() + hid_device_count)
                      / percent_denominator;
    ...
    for (unsigned int detector_idx = 0;
         detector_idx < (unsigned int) device_detectors.size() && detection_is_required.load();
         detector_idx++) {
...
void ResourceManager::Initialize(bool tryConnect,
                                 bool detectDevices,
                                 bool startServer,
                                 bool applyPostOptions)
{
...
void ResourceManager::HidExitCoroutine()
{
    ...
    LOG_DEBUG("[ResourceManager] Closing HID interfaces: %s",
              ((hid_status == 0) ? "Success" : "Failed"));
}
...
void ResourceManager::RunInBackgroundThread(std::function<void()> coroutine)
{
    if (std::this_thread::get_id() == DetectDevicesThread->get_id()) {
        ...
        if (ScheduledBackgroundFunction != nullptr) {
            LOG_WARNING("[ResourceManager] Detection coroutine: assigned a new coroutine when one "
                        "was already scheduled - probably two rescan events sent at once");
        }
    ...
    while (background_thread_running) {
        if (ScheduledBackgroundFunction) {
            ...
            try {
                ...
                LOG_ERROR("[ResourceManager] Unhandled exception in coroutine; e.what(): %s",
                          e.what());
            } catch (...) {
...
void ResourceManager::UpdateDetectorSettings()
{
    ...
    \*-------------------------------------------------*/
    for (unsigned int i2c_detector_idx = 0;
         i2c_detector_idx < (unsigned int) i2c_dimm_device_detectors.size();
         i2c_detector_idx++) {
        detection_string = i2c_dimm_device_detectors[i2c_detector_idx].name.c_str();
    ...
    for (unsigned int i2c_pci_detector_idx = 0;
         i2c_pci_detector_idx < (unsigned int) i2c_pci_device_detectors.size();
    ...
    for (unsigned int hid_wrapped_detector_idx = 0;
         hid_wrapped_detector_idx < (unsigned int) hid_wrapped_device_detectors.size();
    ...
    | need to be saved to the settings                  |
    \*-------------------------------------------------*/
    for (unsigned int detector_idx = 0; detector_idx < (unsigned int) device_detectors.size();
         detector_idx++) {
        detection_string = device_detector_strings[detector_idx].c_str();
