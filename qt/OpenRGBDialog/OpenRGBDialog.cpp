...
void OpenRGBDialog::UpdateDevicesList()
{
    ...
    printf("UpdateDevicesList, controller count %d\r\n", controllers.size());
    ...
    {
        printf("Searching for controller page for controller %d\r\n", controller_idx);
        ...
        for(int tab_idx = 0; tab_idx < ui->DevicesTabBar->count(); tab_idx++)
        {
            ...
            if(dynamic_cast<OpenRGBDevicePage*>(page) != nullptr)
            {
                ...
                {
                    printf("Found controller page in tab bar\r\n");
                    ...
                    | vector                                |
                    \*-------------------------------------*/
                    if(controllers[controller_idx]->flags & CONTROLLER_FLAG_HIDDEN)
                    {
        ...
        if(!found)
        {
            for(std::size_t page_idx = 0; page_idx < hidden_pages.size(); page_idx++)
            {
                ...
                if(controllers[controller_idx] == ((OpenRGBDevicePage*)page)->GetController())
                {
                    printf("Found controller page in hidden pages\r\n");
        ...
        if(!found)
        {
            printf("Not found, creating new page\r\n");
            /*---------------------------------------------*\
            ...
            if(controllers[controller_idx]->flags & CONTROLLER_FLAG_HIDDEN)
            {
                printf("Placing new page in hidden pages\r\n");
                hidden_pages.push_back(NewPage);
            }
            else
            {
                printf("Placing new page in tab bar\r\n");
