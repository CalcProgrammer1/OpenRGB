...
static unsigned int matrix_map_PAD[5][4] =
{
    {  84, 83, 82, 81 },
    {  80, 79, 78, 77 },
    {  76, 75, 74, NA },
...
RGBController_GaiZhongGaiKeyboard::RGBController_GaiZhongGaiKeyboard(GaiZhongGaiKeyboardController* GaiZhongGai_ptr)
{
    GaiZhongGai = GaiZhongGai_ptr;
    ...
    location    = GaiZhongGai->GetDeviceLocation();
    serial      = GaiZhongGai->GetSerialString();
    ...
    for(unsigned int zone_idx = 0; zone_idx < 1; zone_idx++)
    {
        ...
        switch(GaiZhongGai->GetUSBPID())
        {
            ...
            case GaiZhongGai_17_PRO_PID:
                ...
                matrix_width            = 4;
                matrix_map_ptr          = (unsigned int *)&matrix_map_PAD;
                break;
...
void RGBController_GaiZhongGaiKeyboard::DeviceUpdateLEDs()
{
    ...
    {
        colordata[(color_idx*3)+0] = RGBGetGValue(colors[color_idx]);
        colordata[(color_idx*3)+1] = RGBGetRValue(colors[color_idx]);
        colordata[(color_idx*3)+2] = RGBGetBValue(colors[color_idx]);
    }

    GaiZhongGai->SendColors(colordata, sizeof(colordata));
}
