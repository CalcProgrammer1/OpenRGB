/*---------------------------------------------------------*\
| RGBController_MSIMotherboard112.cpp                       |
|                                                           |
...
/**------------------------------------------------------------------*\
    ...
    @detectors DetectMSIMotherboardControllers
    @comment
\*-------------------------------------------------------------------*/

RGBController_MSIMotherboard112::RGBController_MSIMotherboard112(MSIMotherboard112Controller* controller_ptr)
{
...
RGBController_MSIMotherboard112::~RGBController_MSIMotherboard112()
{
    Shutdown();
...
int RGBController_MSIMotherboard112::GetDeviceMode()
{
...
void RGBController_MSIMotherboard112::SetupZones()
{
    /*-----------------------------------------------------*\
    | Set up zones                                          |
    \*-----------------------------------------------------*/
...
void RGBController_MSIMotherboard112::DeviceUpdateLEDs()
{
...
void RGBController_MSIMotherboard112::DeviceUpdateZoneLEDs
    (
    int zone
    )
{
...
void RGBController_MSIMotherboard112::DeviceUpdateSingleLED
    (
    int led
    )
{
...
void RGBController_MSIMotherboard112::DeviceUpdateMode()
{
...
void RGBController_MSIMotherboard112::DeviceSaveMode()
{
     controller->Update(true);
}

void RGBController_MSIMotherboard112::SetupModes()
{
...
void RGBController_MSIMotherboard112::UpdateLed
    (
    int zone,
    int led
    )
...
void RGBController_MSIMotherboard112::SetupMode
    (
    const char      *name,
    MSI_MODE        mod,
    unsigned int    flags
    )
{
    ...
    {
        /*-------------------------------------------------*\
        | For modes without speed this needs to be set to   |
        | avoid bad values in the saved profile which in    |
        | turn corrupts the brightness calculation when     |
        | loading the profile                               |
        \*-------------------------------------------------*/
        Mode.speed      = 0;
        Mode.speed_max  = 0;
...
void RGBController_MSIMotherboard112::GetDeviceConfig()
{
    MSI_MODE       mode;
    MSI_SPEED      speed;
