...
MSIKeyboard1565Controller::MSIKeyboard1565Controller(hid_device *handle, const char *path)
{
    dev         = handle;
    location    = path;
}

MSIKeyboard1565Controller::~MSIKeyboard1565Controller()
{
    hid_close(dev);
}

void MSIKeyboard1565Controller::SetMode
(
    MS_1565_MODE mode,
...
std::string MSIKeyboard1565Controller::GetDeviceName()
{
    wchar_t tname[256];
...
std::string MSIKeyboard1565Controller::GetFWVersion()
{
    /*-----------------------------------------------------*\
...
std::string MSIKeyboard1565Controller::GetDeviceLocation()
{
    return ("HID: " + location);
}

std::string MSIKeyboard1565Controller::GetSerial()
{
