...
NetPacketStatus NetworkServer::ProcessRequest_GetHIDDeviceInfo(NetworkClientInfo* client_info)
{
    ...
    unsigned int                data_size       = 0;
    unsigned int                device_count    = device_info.size();
    ...
    for(unsigned int device_idx = 0; device_idx < device_count; device_idx++)
    {
        ...
        data_size                              += strlen(device_info[device_idx].serial_number.c_str()) + 1;
        data_size                              += sizeof(unsigned short);
        data_size                              += strlen(device_info[device_idx].manufacturer_string.c_str()) + 1;
        data_size                              += sizeof(unsigned short);
        data_size                              += strlen(device_info[device_idx].product_string.c_str()) + 1;
        data_size                              += sizeof(unsigned short);
        data_size                              += strlen(device_info[device_idx].path.c_str()) + 1;
    }
    ...
    for(unsigned int device_idx = 0; device_idx < device_count; device_idx++)
    {
        ...
        unsigned short serial_number_size = strlen(device_info[device_idx].serial_number.c_str()) + 1;
        ...
        unsigned short manufacturer_string_size = strlen(device_info[device_idx].manufacturer_string.c_str()) + 1;
        ...
        unsigned short product_string_size = strlen(device_info[device_idx].product_string.c_str()) + 1;
        ...
        unsigned short path_size = strlen(device_info[device_idx].path.c_str()) + 1;
...
NetPacketStatus NetworkServer::ProcessRequest_RGBController_AddSegment(NetworkClientInfo* client_info, unsigned int data_size, unsigned char* data_ptr, unsigned int controller_id)
{
    ...
    | Add new segment                                       |
    \*-----------------------------------------------------*/
    data_ptr = controllers[controller_idx]->SetSegmentDescription(data_ptr, data_size - (data_ptr - data_start), &new_segment, client_info->client_protocol_version);
...
NetPacketStatus NetworkServer::ProcessRequest_RGBController_ConfigureZone(NetworkClientInfo* client_info, unsigned int data_size, unsigned char* data_ptr, unsigned int controller_id)
{
    ...
    data_ptr = controllers[controller_idx]->SetZoneDescription(data_ptr, data_size - (data_ptr - data_start), &new_zone, client_info->client_protocol_version);
...
NetPacketStatus NetworkServer::ProcessRequest_RGBController_UpdateLEDs(NetworkClientInfo* client_info, unsigned int data_size, unsigned char* data_ptr, unsigned int controller_id)
{
    ...
    | Update colors                                         |
    \*-----------------------------------------------------*/
    data_ptr = RGBController::SetColorDescription(data_ptr, data_size - (data_ptr - data_start), controllers[controller_idx], client_info->client_protocol_version);
